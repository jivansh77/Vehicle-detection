...
def detect_cars():
    try:
        ...
        if len(image_bytes) > 100 * 1024:  # Reduced from 250KB to 100KB
            return jsonify({'error': 'Image too large. Maximum size is 100KB'}), 413
        ...
        _, buffer = cv2.imencode('.jpg', processed_image, [cv2.IMWRITE_JPEG_QUALITY, 30])  # Further reduced quality
